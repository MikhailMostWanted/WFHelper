...
async function main() {
  ...
  const CROP_CANDIDATES = [
    ...
    { label: "candidate-A", x: 0.4, y: 0.38, w: 0.22, h: 0.42 },
    { label: "candidate-B", x: 0.42, y: 0.38, w: 0.2, h: 0.42 },
    { label: "candidate-C", x: 0.4, y: 0.35, w: 0.24, h: 0.45 },
    { label: "candidate-D", x: 0.38, y: 0.35, w: 0.26, h: 0.45 },
    { label: "OLD-PROD", x: 0.34, y: 0.39, w: 0.28, h: 0.49 },
    // Debug script's right crop
    { label: "DEBUG-SCRIPT", x: 0.5, y: 0.38, w: 0.28, h: 0.38 },
  ];
  ...
  for (const file of files) {
    ...
    for (const c of CROP_CANDIDATES) {
      ...
      const parsed = stats
        .map((s) => `${s.positive ? "+" : "-"}${s.value ?? "?"}% ${s.name}`)
        .join(" | ");
      const preview = text.replace(/\r?\n/g, " ↵ ").trim().slice(0, 120);
      const cx = Math.floor(w * c.x),
        cy = Math.floor(h * c.y);
      const cw2 = Math.floor(w * c.w),
        ch2 = Math.floor(h * c.h);
