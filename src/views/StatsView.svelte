...
<script lang="ts">
  ...
  let trackingMounted = false;
  $: if ($statsTab === "tracking") trackingMounted = true;
...
<section class="view active">
  ...
  {#if trackingMounted}
    <div
      ...
      style:display={$statsTab === "tracking" ? undefined : "none"}
      data-stats-tracking
    >
      {#if loading}
      ...
      {:else}
        ...
        >
          {#if importStatus}
            <p class="mb-3 text-xs {importError ? 'text-danger' : 'text-success'}">
              {importStatus}
            </p>
          {/if}
